...
class ResidualFSQ(Module):
    ...
    def get_codes_from_indices(self, indices):
        ...
        # Manual indexing to replace einx.get_at("q [c] d, b n q -> q b n d", self.codebooks, indices)
        # self.codebooks shape: (q, codebook_size, d)
        # indices shape: (b, n, q)
        # Output shape: (q, b, n, d)
        q, codebook_size, d = self.codebooks.shape
        b, n, q_dim = indices.shape
        # Reshape for indexing: (b, n, q) -> (b*n*q,)
        indices_flat = indices.reshape(-1)
        # For each quantizer q, gather codes
        all_codes_list = []
        for q_idx in range(q):
            # Get indices for this quantizer: (b, n)
            q_indices = indices[:, :, q_idx]  # (b, n)
            # Get codebook for this quantizer: (codebook_size, d)
            q_codebook = self.codebooks[q_idx]  # (codebook_size, d)
            # Index into codebook: (b, n, d)
            q_codes = q_codebook[q_indices]  # (b, n, d)
            all_codes_list.append(q_codes)
        # Stack: (q, b, n, d)
        all_codes = torch.stack(all_codes_list, dim=0)
