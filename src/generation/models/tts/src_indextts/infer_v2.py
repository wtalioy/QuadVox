...
os.environ['HF_HUB_CACHE'] = 'cache/IndexTTS-2/hf_cache'
import json
import re
import numpy as np
import librosa
...
class IndexTTS2:
    def __init__(
            self, model_dir="cache/IndexTTS-2", use_fp16=False, device=None,
        ...
        snapshot_download(repo_id="IndexTeam/IndexTTS-2", local_dir=model_dir)
    ...
    def _load_and_cut_audio(self,audio_path,max_audio_length_seconds,sr=None):
        if not sr:
    ...
    def infer(self, spk_audio_prompt, text,
        ...
        if stream_return:
            return self.infer_generator(
                ...
                max_text_tokens_per_segment, stream_return, more_segment_before, **generation_kwargs
            )
        else:
            try:
    ...
    def infer_generator(self, spk_audio_prompt, text,
        ...
        if self.cache_emo_cond is None or self.cache_emo_audio_prompt != emo_audio_prompt:
            if self.cache_emo_cond is not None:
                self.cache_emo_cond = None
                torch.cuda.empty_cache()
            emo_audio, _ = self._load_and_cut_audio(emo_audio_prompt,15,sr=16000)
