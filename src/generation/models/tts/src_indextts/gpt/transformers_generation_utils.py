...
# Try to import cache mappings, provide fallbacks if not available (for newer transformers versions)
try:
    from transformers.generation.configuration_utils import (
    ...
    # Fallback for newer transformers versions where these might not be exported
    # These are typically internal mappings, so we provide minimal fallbacks
    NEED_SETUP_CACHE_CLASSES_MAPPING = {}
    QUANT_BACKEND_CLASSES_MAPPING = {}
...
def _crop_past_key_values(model, past_key_values, max_length):
    """
    Crop past_key_values to the specified length.
    
    This function handles both Cache objects (DynamicCache, StaticCache, etc.) 
    and tuple-based past_key_values for compatibility with different transformers versions.
    
    Args:
        model: The model instance (used for compatibility with old API)
        past_key_values: Cache object or tuple of past key values
        max_length: Maximum length to crop to
    ...
    if isinstance(past_key_values, Cache):
        # For Cache objects, use the crop method
        past_key_values.crop(max_length)
        return past_key_values
    
    # Handle tuple-based past_key_values (legacy format)
    # Format: tuple of tuples, where each inner tuple is (key, value) for a layer
    if isinstance(past_key_values, tuple):
        ...
        for layer_past in past_key_values:
            ...
            # Each layer_past is typically a tuple of (key, value) tensors
            if isinstance(layer_past, tuple) and len(layer_past) == 2:
                key, value = layer_past
                # Crop along the sequence length dimension (usually dim=-2 or dim=2)
                # The exact dimension depends on the tensor shape
                if key.dim() >= 2:
                    # Find the sequence length dimension
                    # Usually it's dim=2 for (batch, heads, seq_len, head_dim) or dim=-2
                    seq_dim = 2 if key.dim() == 4 else -2
                    cropped_key = key[..., :max_length, :] if seq_dim == -2 else key[:, :, :max_length, :]
                    cropped_value = value[..., :max_length, :] if seq_dim == -2 else value[:, :, :max_length, :]
    ...
    # If it's neither Cache nor tuple, return as-is
    return past_key_values
...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        # Check for forced_decoder_ids (deprecated in newer transformers versions)
